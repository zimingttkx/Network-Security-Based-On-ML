...
- **Real capture, measured in CI:** `scripts/verify_real_capture_quality.py` rebuilds the shipped UNSW-NB15 reconstruction (82,523 packets) and runs it through the full pipeline, gating the false-positive rate. Latest runs: detection rate **0.0–0.5%**, false-positive rate **1.8–3.7%** — Kitsune's projections are unseeded so the two rates move between runs, and the capture is rebuilt from flow records with shortened grace periods. Read it as calibration, not accuracy: on this capture the ML stage barely separates attack from normal, and the detections that matter come from the rules you write.
