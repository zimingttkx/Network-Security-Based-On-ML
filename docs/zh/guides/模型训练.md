...
- 批次大小（Batch Size）：每次训练使用的样本数
- 训练轮数（Epochs）：完整遍历数据集的次数
- 优化器（Optimizer）：Adam、SGD或RMSprop

**高级参数**：
- Dropout率：防止过拟合
- L2正则化：权重衰减系数
- 早停耐心值：验证损失不下降的最大轮数
- Batch Normalization：是否使用批归一化

**网络架构**：
- DNN：隐藏层神经元数量（如：128,64,32）
- CNN：卷积滤波器数量和全连接层大小
- LSTM：LSTM单元数量和全连接层大小
- 激活函数：ReLU、Tanh或Sigmoid
...
### 模型文件
- `models/model.pkl` - 最佳模型
- `models/preprocessor.pkl` - 数据预处理器
